...
data2 = loadData2()
kf = KFold(n=data.shape[0], n_folds=10, shuffle=True,random_state=0)
X = data[:,:-2]
...
xdata = np.concatenate((data2[:,5:9],X[:,4:8]),axis=0)
xdata = np.log(xdata+1)
# scaler1 = StandardScaler()
# xdata = scaler1.fit_transform(xdata)
data2[:,5:9] = xdata[:data2.shape[0],:]
X[:,4:8] = xdata[data2.shape[0]:,:]
# data2 = np.log(data2+1)
stackedNetwork = StackedGPNetwork(3)
# add FFMC node
stackedNetwork.createNewNode(0,data2[:,1:5],data2[:,5:6], normalize=True,ARD=False, useGPU=False)
# add DMC node
stackedNetwork.createNewNode(0,data2[:,[1,2,4]],data2[:,6:7], normalize=True,ARD=False, useGPU=False)
# add DC node
stackedNetwork.createNewNode(0,data2[:,[1,4]],data2[:,7:8], normalize=True,ARD=False, useGPU=False)
# add ISI node
stackedNetwork.createNewNode(1,data2[:,[3,5]],data2[:,8:9], normalize=True,ARD=False, useGPU=False)
# add BUI node
...
stackedNetwork.createNewNode(2,data2[:,8:9],data2[:,9:10], normalize=True,ARD=True, useGPU=False)
...
for train_index, test_index in kf:
    ...
    inputSecondGP = trainX[:,[4,5,6,7]]
    kernel1 = GPy.kern.RBF(input_dim=inputSecondGP.shape[1], ARD=False,useGPU=False)
    gp = GPy.models.GPRegression(inputSecondGP, trainY,kernel=kernel1, normalizer=False)
    ...
    sdata = np.concatenate((testX[:,[10]],fmean[:,[0]]),axis=1)
    stv = np.concatenate((np.zeros(testX[:,[10]].shape),fvar[:,[0]]),axis=1)
